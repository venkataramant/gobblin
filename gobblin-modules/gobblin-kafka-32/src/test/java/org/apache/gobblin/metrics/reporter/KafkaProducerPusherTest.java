...
public class KafkaProducerPusherTest {
  ...
  public void test() throws IOException {
    // Test that the scoped config overrides the generic config
	  Optional<Config> optionals=Optional.of(ConfigFactory.parseMap(ImmutableMap.of(
    ...
    Pusher pusher = new KafkaProducerPusher("127.0.0.1:dummy", TOPIC, optionals);
    ...
    Assert.assertEquals(new String(iterator.next().value()), msg1);
    assert(iterator.hasNext());
    Assert.assertEquals(new String(iterator.next().value()), msg2);
