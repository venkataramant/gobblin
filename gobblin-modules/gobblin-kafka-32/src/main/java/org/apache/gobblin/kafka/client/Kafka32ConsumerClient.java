...
/**
 * A {@link GobblinKafkaConsumerClient} that uses kafka 32 consumer client. Use {@link Factory#create(Config)} to create
 * new Kafka32ConsumerClients. The {@link Config} used to create clients must have required key {@value #GOBBLIN_CONFIG_VALUE_DESERIALIZER_CLASS_KEY}
 *
 * @param <K> Message key type
 * @param <V> Message value type
 */
@Slf4j
public class Kafka32ConsumerClient<K, V> extends AbstractBaseKafkaConsumerClient {
  ...
  private static final String DEFAULT_GROUP_ID = "kafka32";
  ...
  private Kafka32ConsumerClient(Config config) {
    super(config);
  ...
  public Kafka32ConsumerClient(Config config, Consumer<K, V> consumer) {
    super(config);
  ...
  public Iterator<KafkaConsumerRecord> consume() {
    try {
      ...
      return Iterators.transform(consumerRecords.iterator(), input -> {
        try {
          return new Kafka32ConsumerRecord(input);
        } catch (Throwable t) {
  ...
  /**
    * A factory class to instantiate {@link Kafka32ConsumerClient}
   */
  public static class Factory implements GobblinKafkaConsumerClientFactory {
    ...
    public GobblinKafkaConsumerClient create(Config config) {
      return new Kafka32ConsumerClient(config);
    }
  }

  /**
   * A record returned by {@link Kafka32ConsumerClient}
   *
   * @param <K> Message key type
   * @param <V> Message value type
   */
  ...
  @ToString
  public static class Kafka32ConsumerRecord<K, V> extends BaseKafkaConsumerRecord implements
      DecodeableKafkaRecord<K, V> {
    private final ConsumerRecord<K, V> consumerRecord;

    public Kafka32ConsumerRecord(ConsumerRecord<K, V> consumerRecord) {
      // Kafka 32 consumerRecords do not provide value size.
