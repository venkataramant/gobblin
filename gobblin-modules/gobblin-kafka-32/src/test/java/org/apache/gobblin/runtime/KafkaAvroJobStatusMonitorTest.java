...
import org.apache.gobblin.kafka.client.Kafka32ConsumerClient;
...
public class KafkaAvroJobStatusMonitorTest {
  ...
  public void testProcessMessageForSuccessfulFlow() throws IOException, ReflectiveOperationException {
    ...
    Config config = ConfigFactory.empty().withValue(ConfigurationKeys.KAFKA_BROKERS, ConfigValueFactory.fromAnyRef("localhost:0000"))
        .withValue(Kafka32ConsumerClient.GOBBLIN_CONFIG_VALUE_DESERIALIZER_CLASS_KEY, ConfigValueFactory.fromAnyRef("org.apache.kafka.common.serialization.ByteArrayDeserializer"))
    ...
    Iterator<ConsumerRecord<byte[], byte[]>> iterator = this.kafkaTestHelper.getIteratorForTopic(TOPIC);

    ConsumerRecord<byte[], byte[]> messageAndMetadata = iterator.next();
  ...
  public void testProcessMessageForFailedFlow() throws IOException, ReflectiveOperationException {
    ...
    byte[] undecodeableMessage = Arrays.copyOf(messageAndMetadata.value(),
        messageAndMetadata.value().length - 1);
    ...
    jobStatusMonitor.processMessage(new Kafka32ConsumerClient.Kafka32ConsumerRecord(undecodeableRecord));
  ...
  private DecodeableKafkaRecord convertMessageAndMetadataToDecodableKafkaRecord(ConsumerRecord<byte[], byte[]> record) {
    ConsumerRecord consumerRecord = new ConsumerRecord<>(TOPIC, record.partition(), record.offset(), record.key(), record.value());
    return new Kafka32ConsumerClient.Kafka32ConsumerRecord(consumerRecord);
  }
